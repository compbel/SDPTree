...
simulTypes = {'phylobench'};
% simulTypes = {'points','tree_seq','RIM','RDSM'};


for st = 1:length(simulTypes)
    simulType = simulTypes{st};
    ...
    algParam.nMergeCher = 4;
    
    resFold = [outputFold filesep 'resSim_' algParam.maxDepthType 'H_Ch' int2str(algParam.nMergeCher) '_' simulType];
    % resFold = [outputFold filesep 'resSim_' algParam.maxDepthType 'H_Ch' int2str(algParam.nMergeCher) '_time_' simulType];
    if ~isfolder(resFold)
        mkdir(resFold);
    end
    ...
    for n = ns
        ...
        if strcmp(simulType,'phylobench')
            fileType = fullfile([inputFold filesep 'phylobench' filesep 'nexus-' int2str(n)], '*.nex');
            files = dir(fileType);
            nTests = length(files);
        end
        K = n-1;
        time_n = zeros(1,nTests);
        parfor i = 1:nTests
            try
                ...
                if strcmp(simulType,'RIM')
                    ...
                    matrFold = [inputFold filesep 'randMatr' filesep testID];
                ...
                if matrFromFile
                    matrFile = [inputFold filesep 'matr_' simulType filesep 'matr_' int2str(n) '_' int2str(i) '.mat'];
                    loadStruct = load(matrFile);
                    D = loadStruct.D;
                end
