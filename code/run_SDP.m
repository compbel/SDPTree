...
for n = ns
    ...
    for i = 1:nTests
        try
            ...
            [F_FME,T_FME,rho_FME,z_FME,tm_FME, nRear_FME] = runFME(fmefolder,auxfiles,auxfiles,i,D,T0);
            [F_FME_NJ,T_FME_NJ,~,~,~, nRear_FME_NJ] = runFME(fmefolder,auxfiles,auxfiles,i,D,[]);
            [F_NJ, T_NJ] = runNJ(D,algParam);
        
            algParamCurr = algParam;
            algParamCurr.heurRuleCh = 'edge';
            t = tic
            out = solveSDP(D,algParamCurr);
            tm = toc(t);
            F_SDP = out.F;
            T_SDP = out.T;
            [F_SDP_SPR,T_SDP_SPR,~,~,~, nRearSPR] = runFME(fmefolder,auxfiles,auxfiles,i,D,T_SDP,algParam);
    
            algParamCurr.heurRuleCh = 'twin';
            t = tic
            out1 = solveSDP(D,algParamCurr);
            tm1 = toc(t);
            F_SDP1 = out1.F;
            T_SDP1 = out1.T;
            [F_SDP_SPR1,T_SDP_SPR1,~,~,~, nRearSPR1] = runFME(fmefolder,auxfiles,auxfiles,i,D,T_SDP1);
